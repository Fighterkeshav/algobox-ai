...
export function EpicCyberVisualizer({ state, mode }: EpicCyberVisualizerProps) {
    ...
    // GSAP Entrance for UI Interface
    useEffect(() => {
        if (!containerRef.current) return;

        const corners = containerRef.current.querySelectorAll('.corner-dec');
        const badge = containerRef.current.querySelector('.mode-badge');

        // Animate corners expanding
        gsap.fromTo(corners,
            { scale: 0, opacity: 0 },
            { scale: 1, opacity: 1, duration: 1, stagger: 0.1, ease: "power3.out", delay: 0.5 }
        );

        // Animate badge entrance
        gsap.fromTo(badge,
            { y: -20, opacity: 0 },
            { y: 0, opacity: 1, duration: 0.8, ease: "back.out(1.7)", delay: 1 }
        );

    }, []);

    // Status Badge Glitch Effect
    ...
    return (
        <div ref={containerRef} className="relative w-full h-full overflow-hidden bg-slate-950 rounded-xl border border-slate-800 shadow-2xl">
            {/* Ambient gradient background */}
            <div className={`absolute inset-0 transition-all duration-1000 ${mode === "attack"
                ? "bg-gradient-to-br from-slate-950 via-red-950/20 to-slate-950"
                : "bg-gradient-to-br from-slate-950 via-emerald-950/20 to-slate-950"
                }`} />

            {/* Animated scan lines */}
            <div className="absolute inset-0 pointer-events-none z-0">
                <div className="absolute inset-0 bg-[linear-gradient(transparent_50%,rgba(0,0,0,0.3)_50%)] bg-[length:100%_4px] animate-scan opacity-30" />
            </div>
            ...
            {/* Corner decorations */}
            <div className="corner-dec absolute top-4 left-4 w-16 h-16 border-l-2 border-t-2 border-blue-500/40 rounded-tl-lg pointer-events-none z-30" />
            <div className="corner-dec absolute top-4 right-4 w-16 h-16 border-r-2 border-t-2 border-blue-500/40 rounded-tr-lg pointer-events-none z-30" />
            <div className="corner-dec absolute bottom-4 left-4 w-16 h-16 border-l-2 border-b-2 border-blue-500/40 rounded-bl-lg pointer-events-none z-30" />
            <div className="corner-dec absolute bottom-4 right-4 w-16 h-16 border-r-2 border-b-2 border-blue-500/40 rounded-br-lg pointer-events-none z-30" />

            {/* Mode indicator */}
            <div
                ref={badgeRef}
                className={`mode-badge absolute top-6 right-6 px-4 py-2 rounded border text-xs font-bold uppercase tracking-widest z-40 shadow-lg backdrop-blur-sm ${mode === "attack"
                    ? "bg-red-500/10 text-red-500 border-red-500/50 shadow-red-900/20"
                    : "bg-emerald-500/10 text-emerald-500 border-emerald-500/50 shadow-emerald-900/20"
                    }`}>
                <span className="flex items-center gap-2">
                    <span className={`relative flex h-2 w-2`}>
                        <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${mode === "attack" ? "bg-red-400" : "bg-emerald-400"}`}></span>
                        <span className={`relative inline-flex rounded-full h-2 w-2 ${mode === "attack" ? "bg-red-500" : "bg-emerald-500"}`}></span>
                    </span>
                    {mode === "attack" ? "THREAT DETECTED" : "SYSTEM SECURE"}
                </span>
            </div>
        </div>
    );
