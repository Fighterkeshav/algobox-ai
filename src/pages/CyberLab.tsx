...
import { Play, Pause, StepForward, StepBack, Shield, ShieldAlert, BadgeCheck, ChevronDown, Monitor, Code } from "lucide-react";
...
import { CyberAssignment } from "@/components/cyber-assignment/CyberAssignment";
    ...
    const handleTabChange = (value: string) => {
        ...
        if (containerRef.current) {
            animate(containerRef.current, {
                opacity: [1, 0],
                translateY: [0, 10],
                // easing: 'inQuad', // Standard Easing for v4 likely changed, stick to defaults or use linear if unsure, or try 'easeInQuad' as generic text if supported. 
                // Documentation implies specific easing modules but string names might still work via parser. 
                // Let's use simple duration for now to be safe or assuming 'inQuad' maps if registered.
                easing: 'inQuad',
                duration: 200,
            }).then(() => {
                setActiveTab(value);
            ...
            <Tabs value={activeTab} onValueChange={handleTabChange} className="h-full flex flex-col">
                <div className="flex items-center justify-between mb-4">
                    <TabsList className="bg-slate-900 border border-slate-700">
                        ...
                        <TabsTrigger value="practice" className="data-[state=active]:bg-slate-800">
                            <Code className="w-4 h-4 mr-2" />
                            Assignment Mode
                        </TabsTrigger>
                    </TabsList>

                    {/* Lab Selector (Only relevant for Simulation for now, but kept global) */}
                ...
                <div ref={containerRef} className="flex-1 min-h-0 flex flex-col">
                    <TabsContent value="simulation" className="flex-1 min-h-0 mt-0 data-[state=inactive]:hidden">
                    ...
                    <TabsContent value="practice" className="flex-1 min-h-0 mt-0 data-[state=inactive]:hidden">
                        <CyberAssignment labId={currentLabId} />
                    </TabsContent>
                </div>
            </Tabs>
        </div>
